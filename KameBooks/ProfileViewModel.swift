...
final class ProfileViewModel: ObservableObject {
    ...
    @MainActor func updateUser() async {
        ...
        let task = Task(priority: .utility) {
            try await networkPersistance.updateUser(user: UserModel(name: name, email: mail, location: address, role: userData?.role ?? ""))
        }
        switch await task.result {
        case .success(_):
            KameBooksKeyChain.shared.user = UserModel(name: name, email: mail, location: address, role: userData?.role ?? "")
